...
class Meting
{
    ...
    public function song($id)
    {
        switch ($this->server) {
            ...
            case 'migu':
                $api = array(
                    'method' => 'GET',
                    'url'    => 'https://app.c.nf.migu.cn/MIGUM2.0/v1.0/content/resourceinfo.do',
                    'body'   => array( 
                        'copyrightId'   => $id,
                        'resourceType'      => 2,
                        'needSimple' => '00', 
                    ),  
                    'decode' => 'migu_resourceinfo_decode',
                );
                break; 
        }
    ...
    private function migu_resourceinfo_decode($result)
    {
        $data = json_decode($result, true);
